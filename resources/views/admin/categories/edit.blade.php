...
<head>
    ...
    <title>تعديل الفئة - {{ $category->name }} - MHD Admin</title>
            ...
            <div class="col-lg-8">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <a href="{{ route('admin.categories.index') }}" class="btn btn-outline-secondary">
                        <i class="bi bi-arrow-right me-2"></i>العودة للفئات
                    </a>
                </div>
                    ...
                    <div class="card-body p-5">
                        <form method="POST" action="{{ route('admin.categories.update', $category) }}">
                            ...
                            <div class="row g-4">
                                ...
                                <div class="col-md-6">
                                    <label class="form-label">لون الفئة</label>
                                    <div class="input-group">
                                        <input type="color" name="color" class="form-control form-control-color @error('color') is-invalid @enderror" value="{{ old('color', $category->color ?? '#1F3C88') }}" title="اختر لون">
                                        <span class="input-group-text">معاينة</span>
                                    </div>
                                    <div class="color-preview mt-2 d-inline-block" style="background-color: {{ old('color', $category->color ?? '#1F3C88') }};" id="colorPreview"></div>
                                    @error('color')
                                        <div class="invalid-feedback d-block">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label"><strong>عدد المنتجات</strong></label>
                                    <input type="text" class="form-control" value="{{ $category->products_count ?? 0 }}" disabled>
                                    <small class="text-muted">عدد المنتجات في هذه الفئة</small>
                                </div>
                                <div class="col-12 d-grid gap-2 d-md-flex justify-content-end">
                                    ...
                                    <button type="submit" class="btn btn-primary px-4">تحديث الفئة</button>
                                </div>
                            </div>
                        </form>
                    </div>
