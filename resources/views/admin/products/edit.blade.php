...
<head>
    ...
    <title>تعديل المنتج - MHD Print Lab Admin</title>
    ...
    <style>
        ...
        .current-media { border: 2px solid #e9ecef; border-radius: 8px; }
...
<body>
                        ...
                        <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row g-4">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="number" name="discount_percentage" class="form-control" step="0.01" min="0" max="100" value="{{ old('discount_percentage', $product->discount_percentage ?? 0) }}" placeholder="0">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <small class="text-muted">اتركه فارغ للاحتفاظ بالحالي</small>
                                    @if($product->image)
                                        <div class="mt-2 current-media p-2">
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea name="additional_info" rows="3" class="form-control">{{ old('additional_info', $product->additional_info) }}</textarea>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="col-md-6 mt-4">
                                        ...
                                        <input type="number" name="stock_per_variant" class="form-control" value="{{ old('stock_per_variant', $product->stock_per_variant ?? 10) }}" min="0" placeholder="10">
                                        <small class="text-muted">عدد القطع المتاحة لكل variant</small>
                                    </div>
                                    <div class="col-md-6 mt-4">
                                        <label class="form-label fw-bold">صور الـ variants (JSON)</label>
                                        <textarea name="variant_images" class="form-control" rows="3" placeholder='{"#FF0000_M": "path/to/red-medium.jpg"}'>{{ old('variant_images', $product->variant_images ?? '') }}</textarea>
                                        <small class="text-muted">صيغة: {color_size: image_path}</small>
                                    </div>

                                <div class="col-12">
                                    <label class="form-label">لماذا تختاره؟</label>
                                    <textarea name="why_choose_it" rows="4" class="form-control">{{ old('why_choose_it', $product->why_choose_it) }}</textarea>
                                ...
                                <div class="row mb-4">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" name="materials" class="form-control" value="{{ old('materials', $product->materials) }}" placeholder="مثال: قطن 100% عضوي">
                                        <small class="text-muted">المواد المستخدمة في المنتج</small>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label fw-semibold">الصيانة</label>
                                        <input type="text" name="care_instructions" class="form-control" value="{{ old('care_instructions', $product->care_instructions) }}" placeholder="مثال: غسيل بارد، تجفيف بالهواء">
                                        <small class="text-muted">تعليمات الصيانة والعناية</small>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label fw-semibold">المنشأ</label>
                                        <input type="text" name="origin" class="form-control" value="{{ old('origin', $product->origin) }}" placeholder="مثال: صنع في سوريا وتركيا">
                                        <small class="text-muted">بلد المنشأ أو المصنع</small>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label fw-semibold">الضمان</label>
                                        <input type="text" name="warranty" class="form-control" value="{{ old('warranty', $product->warranty) }}" placeholder="مثال: جودة عالية، ضمان سنة">
    ...
    <script>
        let colors = {{ json_encode(old('available_colors', $product->available_colors ?? [])) }};
        let sizes = {{ json_encode(old('sizes_available', $product->sizes_available ?? [])) }};
